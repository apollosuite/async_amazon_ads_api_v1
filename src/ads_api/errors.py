...
class AmazonAdsAPIError(AmazonAdsError):
    ...
    def __init__(self, response: httpx.Response, message: str = "") -> None:
        ...
        if isinstance(self.error_body, dict):
            error_detail = self.error_body["message"] if "message" in self.error_body else ""
            if not error_detail and "details" in self.error_body:
                error_detail = str(self.error_body["details"])
