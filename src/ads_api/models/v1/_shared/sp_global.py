...
type SPGlobalAdProduct = Literal["SPONSORED_PRODUCTS",]  # Sponsored Products ad product.
"""
Supported values:
...
type SPGlobalCreateState = Literal[
...
type SPGlobalDeliveryStatus = Literal[
...
type SPGlobalProductIdType = Literal[
...
type SPGlobalUpdateState = Literal[
    "ENABLED",  # The object is set active by user and eligible for delivery.
    "PAUSED",  # The object is stopped by user and not eligible for delivery.
]
"""
...
class SPGlobalError(LenientModel):
    code: SPGlobalErrorCode | str = Field(description="""
Supported values:
- `ACTION_NOT_SUPPORTED`: The request is not supported.
- `ACTIVE_RESOURCE_LIMIT_EXCEEDED`: Too many live resources. Remove resources and try again.
- `ARCHIVED_PARENT_CANNOT_CREATE`: New resources cannot be created within an archived parent.
