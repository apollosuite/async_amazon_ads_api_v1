...
type SPAdProduct = Literal["SPONSORED_PRODUCTS"]
"""
Supported values:
...
type SPDeliveryStatus = Literal["DELIVERING", "NOT_DELIVERING", "UNAVAILABLE"]
"""
Supported values:
...
type SPProductIdType = Literal["ASIN", "SKU"]
"""
Supported values:
...
type SPUpdateState = Literal["ENABLED", "PAUSED"]
"""
The user defined state for the resource. For ADSP, campaign and ad group resources can only be created in the PAUSED state and must be updated to ENABLED to activate for delivery
...
class SPStatus(LenientModel):
    ...
    deliveryStatus: SPDeliveryStatus | str
