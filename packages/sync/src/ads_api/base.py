"""Shared HTTP session and base resource class (Synchronous)."""
...
class ClientContext:
    ...
    def get_client(self) -> httpx.Client:
        if self._client is None:
            self._client = httpx.Client(base_url=self.config.base_url, timeout=httpx.Timeout(self.config.timeout))
        return self._client
    ...
    def close(self) -> None:
        ...
        if self.config._token_manager is not None:
            self.config._token_manager.close()
...
class BaseResource:
    ...
    def _request(
        ...
        *,
        ...
        client = self._ctx.get_client()
        if self._ctx.config._token_manager is not None:
            token: str = self._ctx.config.refresh_access_token()
        else:
            ...
            try:
                ...
                if resp.is_error:
                    if resp.status_code == 401 and self._ctx.config._token_manager is not None and attempt == 0:
                        token = self._ctx.config.refresh_access_token(force=True)
