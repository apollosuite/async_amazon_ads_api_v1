"""Token cache implementations — file-based and Redis-backed (Synchronous)."""
...
class BaseTokenCache(ABC):
    @abstractmethod
    def read(self) -> TokenData | None: ...

    @abstractmethod
    def write(self, data: TokenData) -> None: ...
...
class RedisTokenCache(BaseTokenCache):
        ...
        *,
        ...
        if redis_client is not None:
            ...
            try:
                ...
                raise ImportError(
                    "Redis support requires the 'redis' extra: pip install amazon-ads-api-v1[redis]"
                ) from None
    ...
    def write(self, data: TokenData) -> None:
        ...
        if ttl > 0:
            self._client.set(self._key, json.dumps(payload), ex=ttl)
        else:
